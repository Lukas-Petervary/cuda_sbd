...
#include "sbd/chemistry/tpb_k/kernel_mult.h"
#endif
...
#pragma omp parallel for
        ...
        for (size_t task = 0; task < helper.size(); task++) {
          ...
          std::cout << " " << T[i];
                ...
                if (helper[task].taskType == 2) { // beta range are same
                    for (size_t ia = ia_start; ia < ia_end; ia += num_threads) {
                        for (size_t ib = helper[task].braBetaStart; ib < helper[task].braBetaEnd; ib++) {
                            ...
                            // single alpha excitation
                            for (size_t j = 0;
                                 j < helper[task].SinglesFromAlphaLen[ia - helper[task].braAlphaStart]; j++) {
                                size_t ja = helper[task].SinglesFromAlphaSM[ia - helper[task].braAlphaStart][j];
                                size_t ketIdx = (ja - helper[task].ketAlphaStart) * ketBetaSize
                                                + ib - helper[task].ketBetaStart;
                                ElemT eij = OneExcite(DetI, bit_length,
                                                      helper[task].SinglesAlphaCrAnSM[ia - helper[task].braAlphaStart][
                                                              2 * j + 0],
                                                      helper[task].SinglesAlphaCrAnSM[ia - helper[task].braAlphaStart][
                                                              2 * j + 1],
                                                      I1, I2);
                            ...
                            // double alpha excitation
                            for (size_t j = 0;
                                 j < helper[task].DoublesFromAlphaLen[ia - helper[task].braAlphaStart]; j++) {
                                size_t ja = helper[task].DoublesFromAlphaSM[ia - helper[task].braAlphaStart][j];
                                size_t ketIdx = (ja - helper[task].ketAlphaStart) * ketBetaSize
                                                + ib - helper[task].ketBetaStart;
                                ElemT eij = TwoExcite(DetI, bit_length,
                                                      helper[task].DoublesAlphaCrAnSM[ia - helper[task].braAlphaStart][
                                                              4 * j + 0],
                                                      helper[task].DoublesAlphaCrAnSM[ia - helper[task].braAlphaStart][
                                                              4 * j + 1],
                                                      helper[task].DoublesAlphaCrAnSM[ia - helper[task].braAlphaStart][
                    ...
                    for (size_t ia = ia_start; ia < ia_end; ia += num_threads) {
                        for (size_t ib = helper[task].braBetaStart; ib < helper[task].braBetaEnd; ib++) {
                            ...
                            // single beta excitation
                            for (size_t j = 0;
                                 j < helper[task].SinglesFromBetaLen[ib - helper[task].braBetaStart]; j++) {
                                size_t jb = helper[task].SinglesFromBetaSM[ib - helper[task].braBetaStart][j];
                                size_t ketIdx = (ia - helper[task].ketAlphaStart) * ketBetaSize
                                                + jb - helper[task].ketBetaStart;
                                ElemT eij = OneExcite(DetI, bit_length,
                                                      helper[task].SinglesBetaCrAnSM[ib - helper[task].braBetaStart][
                                                              2 * j + 0],
                                                      helper[task].SinglesBetaCrAnSM[ib - helper[task].braBetaStart][
                                                              2 * j + 1],
                                                      I1, I2);
                            ...
                            // double beta excitation
                            for (size_t j = 0;
                                 j < helper[task].DoublesFromBetaLen[ib - helper[task].braBetaStart]; j++) {
                                size_t jb = helper[task].DoublesFromBetaSM[ib - helper[task].braBetaStart][j];
                                size_t ketIdx = (ia - helper[task].ketAlphaStart) * ketBetaSize
                                                + jb - helper[task].ketBetaStart;
                                ElemT eij = TwoExcite(DetI, bit_length,
                                                      helper[task].DoublesBetaCrAnSM[ib - helper[task].braBetaStart][
                                                              4 * j + 0],
                                                      helper[task].DoublesBetaCrAnSM[ib - helper[task].braBetaStart][
                                                              4 * j + 1],
                                                      helper[task].DoublesBetaCrAnSM[ib - helper[task].braBetaStart][
                    ...
                    for (size_t ia = ia_start; ia < ia_end; ia += num_threads) {
                        for (size_t ib = helper[task].braBetaStart; ib < helper[task].braBetaEnd; ib++) {
                            ...
                            for (size_t j = 0;
                                ...
                                size_t ja = helper[task].SinglesFromAlphaSM[ia - helper[task].braAlphaStart][j];
                                for (size_t k = 0;
                                     k < helper[task].SinglesFromBetaLen[ib - helper[task].braBetaStart]; k++) {
                                    size_t jb = helper[task].SinglesFromBetaSM[ib - helper[task].braBetaStart][k];
                                    size_t ketIdx = (ja - helper[task].ketAlphaStart) * ketBetaSize
                                                    + jb - helper[task].ketBetaStart;
                                    ElemT eij = TwoExcite(DetI, bit_length,
                                                          helper[task].SinglesAlphaCrAnSM[ia -
                                                                                          helper[task].braAlphaStart][
                                                                  2 * j + 0],
                                                          helper[task].SinglesBetaCrAnSM[ib -
            ...
            if (helper[task].taskType == 0 && task != helper.size() - 1) {
                ...
                Mpi2dSlide(R, T, adet_comm_size, bdet_comm_size, adetslide, bdetslide, b_comm);
                auto time_slid_end = std::chrono::high_resolution_clock::now();
                auto time_slid_count = std::chrono::duration_cast<std::chrono::microseconds>(
                        time_slid_end - time_slid_start).count();
                time_slid += 1.0e-6 * time_slid_count;
            }
    ...
    void gpuMult(const std::vector <ElemT> &hii,
        ...
        cuda_impl::gpuVecH2D(gpu_ctx, Wb);
        for (size_t task = 0; task < helper.size(); task++) {
            cuda_impl::gpuKetVecH2D(gpu_ctx, T);
            cuda_impl::gpuSpMV(gpu_ctx, static_cast<int>(task));
        ...
        cuda_impl::gpuVecD2H(gpu_ctx, Wb);
